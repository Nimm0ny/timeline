...
- 依赖：仅三项登记例外——`CodeMirror 6`（无感 markdown 编辑）、`AntV X6`（W4 思维导图引擎，含 `@antv/x6` + `@antv/x6-plugin-history` + `@antv/x6-plugin-selection`，用于自由坐标拖拽、连线跟随与编辑历史；导图文件互通基线 = app-native JSON + Markdown，旧 `simple-mind-map` 数据只做读取兼容）与 `@tanstack/vue-virtual`（中栏/右栏/移动端加载性能专项虚拟化基础设施，用于线性视图窗口化、画廊按行虚拟化、看板列内虚拟化）；除此之外不新增依赖；不改 `package-lock.json`（除非确有依赖变更）；不做无关重构/格式化。
