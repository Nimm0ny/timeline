...
- **Goal**：加**第三个入口**——鼠标按住**中栏工具条（`.tl-bar`）或右栏操作条（`.actionbar`）的空白区**水平拖拽，跨过对面面板中线后松手，即翻转 `detailPosition`。直接操纵、可发现、零新状态。
- **Non-goals**：
  - 不做左栏（功能栏）拖拽——`navPosition` 已有设置入口，且左栏顶部是品牌区/ribbon，无干净空白拖拽区，硬加只扩大误触面。
  - 不做自由排序/多槽位停靠（只有两个可交换槽位，本质是一个布尔开关）。
...
- **Scope（文件所有权见 §6）**：新增 1 composable + 1 测试文件；改 `TimelinePage.vue`、`TimelineFeed.vue`、`EventDetailPane.vue`、`timeline-notes.css`；本文档 + spec §3 一行交叉引用。无后端改动、无新依赖、无新图标。
...
| 起手位置 | 拖拽物 | 目标槽位 | 松手效果 |
|---|---|---|---|
| 中栏 `.tl-bar` 空白区 | 「笔记列表」面板 | 详情面板当前占据的轨道 | `detailPosition` 翻转（edge↔center） |
| 右栏 `.actionbar` 空白区 | 「笔记详情」面板 | 中栏（feed/思维导图）当前占据的轨道 | 同上（同一开关，两侧对称） |

- 两个面板互为唯一目标，所以手势不需要「放到哪」的选择——只需要「是否跨线」的判定。
- 与 `navPosition` 正交：四种 2×2 形态下手势都可用，几何按当前形态计算（§3）。
...
| 提交阈值 | 对面面板**中线**（penetration > 对面宽度/2，§3.2） | 只碰边界不算；越过中线后继续深入/超出对面远端仍视为 armed |
...
| 拖拽物 | ghost 图标（`TimelineLucideIcon` 既有名，无新增） | ghost 文案 |
|---|---|---|
| 中栏面板 | `list`（List） | `笔记列表` |
| 右栏面板 | `note`（NotebookText） | `笔记详情` |

两个图标名均已在 spec §4 登记，**本手势不新增图标**；ghost 内图标同样经 `TimelineLucideIcon.vue`（图标纪律）。

## 3. 几何与判定数学

### 3.1 面板水平 span（dragstart 时计算一次，拖拽期间不再读布局）

记 `W = window.innerWidth`、`L = paintedLeft()`（compact desktop 下按 workspaceStyle 同款 clamp 220–240）、`R = state.rightWidth`：

| 形态 | 详情 span | 中栏（feed/mm）span |
|---|---|---|
| edge + nav-left | `[W−R, W]` | `[L, W−R]` |
| edge + nav-right | `[0, R]` | `[R, W−L]` |
| center + nav-left | `[L, L+R]` | `[L+R, W]` |
| center + nav-right | `[W−L−R, W−L]` | `[0, W−L−R]` |

（与 `relatedPreviewPosition` / resizer 的既有数学同源；实现为纯函数 `paneSpans(layout)`，单测覆盖四形态。）

### 3.2 armed 判定（提交规则）

- 共享边界 `B` = 被拖面板与对面面板相邻的那条边；穿透深度 `penetration(x)` = 指针越过 `B` 进入对面方向的距离（未越过 = 0）。
- `armed ⇔ penetration(x) > 对面面板宽度 / 2`。
- 越过对面远端（overshoot）时 penetration 继续增大，**仍为 armed**——拖过头显然意图明确。
- 该判定是纯函数 `isArmed(x, spans, draggedPane)`，只做两次减法一次比较，每帧成本可忽略；且**不读 DOM**（span 在 dragstart 缓存）。

## 4. 误触防线（四道，纵深）

| # | 防线 | 机制 | 兜住的场景 |
|---|---|---|---|
| 1 | **起手区域白名单** | 仅 `event.target === 工具条本体` 或 `.spacer` 才可能起手（**白名单**而非黑名单排除，天然覆盖未来新增按钮） | 点按钮/搜索框/弹层/标题文字/上下文 chip 永不触发 |
| 2 | **死区 + 轴向意图** | 位移 < 6px = 点击（空白区点击本无行为，保持无行为）；首超死区时 `|dy|>|dx|` 即放弃 | 手抖、想滚动/竖向手势 |
| 3 | **提交阈值 = 对面中线** | 只碰边界不算；未 armed 松手 = cancel，**零副作用** | 拖了一半反悔、无意识小幅拖动 |
| 4 | **随时可逃** | Esc / 窗口失焦 / 窗口 resize / pointercancel 一律 cancel；`pointerType==='touch'`、非主键、`isMobile`、`right-closed` 不启用 | 拖到一半想放弃、异常环境 |

另：pointerdown 只在白名单命中后 `preventDefault()`（防文本划选/焦点抖动），不影响按钮等正常元素的默认行为。
...
| 文件 | 改动 | 预估行数 |
|---|---|---|
| `ui/src/composables/usePaneSwapDrag.js` **新增** | 状态机 + 纯几何函数（`paneSpans` / `dragIntent` / `penetration` / `isArmed` 具名导出供单测）；composable 接 `{ isEnabled, getLayout, onCommit }`，返回 `{ onPaneDragStart, dragging, draggedPane, armed, targetRect, bindGhostEl }`；window 级 `pointermove/pointerup/keydown/blur/resize` 监听，沿用 `startResize` 的 cleanup 模式 | ~120 |
| `ui/src/pages/TimelinePage.vue` | 实例化 composable（`getLayout` 复用既有 navRight/detailCenter/paintedLeft/rightWidth；`onCommit` = 既有 `updateDetailPosition`）；模板挂两子组件的 `@pane-drag-start`；渲染 ghost 与目标高亮两个叠加元素；workspace 绑 `pane-dragging` 类 | ~40 |
| `ui/src/components/timeline-notes/TimelineFeed.vue` | `.tl-bar` 上 `@pointerdown="onBarPointerDown"`：白名单命中（bar 本体 / `.spacer`）→ `emit('pane-drag-start', { pane:'feed', x, y, pointerType, button })`；组件不含任何互换逻辑 | ~15 |
| `ui/src/components/timeline-notes/EventDetailPane.vue` | 同上，`.actionbar` → `pane:'detail'` | ~15 |
| `ui/src/styles/timeline-notes.css` | `.pane-drag-ghost` / `.pane-swap-target` / grab·grabbing 光标 / `pane-dragging` 期间 `user-select:none`，全走令牌 | ~45 |
| `ui/tests/paneSwapDrag.test.js` **新增** | §10.1 用例 | ~110 |
| `docs/obsidian-minimal-implementation-spec.md` §3 | 「栏序 2×2」行追加一句：中/右栏可在工具条空白区拖拽互换（指向本文档） | 1 |

合计 ≈ 6 代码文件 / ~350 行（含测试），低于拆分阈值（500 行 / 12 文件）。**无后端改动、无新依赖、无新图标、不动 `package-lock.json`。**
