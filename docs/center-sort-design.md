...
## 3. 核心模型：单一状态 + 方向通用·字段夹取

**状态**（页面所有，见 §6）：

```
sort = { field, dir }
  field: 'time' | 'title' | 'created' | 'updated' | '<自定义列 key>'
  dir:   1 (升序 / 正序) | -1 (降序 / 倒序)
  默认:  { field: 'time', dir: 1 }        // 与今日一致，零行为变化
```

**方向通用、字段夹取**——切视图永不丢方向，字段能用则用、不能用则回落 `time`：

- 在**表格**里按「类型」降序 → 切到**时间线** → 自动落成「时间降序」（字段不适用则弃、方向 `-1` 留着）。
- 在**时间线**设「倒序」→ 切到**列表** → 列表继承「时间降序」。

由 `clampSortForView(sort, view, columns)` 纯函数裁决：目标视图不支持 `field` 时，`field → 'time'`、`dir` 原样保留；`field` 指向一个已删除的自定义列时同样回落 `time`。
...
| 视图 | 排序入口 | 可排字段 | 底层逻辑 |
|---|---|---|---|
| **timeline 时间线** | 工具栏排序按钮 → 2 行 popover（`时间正序 ↑` / `时间倒序 ↓`） | **仅 time** | 倒序 = **反转 era 组序 + 组内序**，分组结构不变（§5 的沉底规则仍生效） |
| **outline 大纲** | 同时间线 | 仅 time | 同上（era→event 两层整体翻转） |
| **table 表格** | **点列头**（主）+ 工具栏菜单（镜像同一状态） | time / title / **所有可见自定义列** | 去分组，扁平按 comparator 排；点激活列头翻 `dir` |
| **list 列表** | 工具栏菜单 | time / title / created / updated | 去分组，扁平排（**不走 era 分桶**） |
| **gallery 画廊** | 工具栏菜单 | time / title / created / updated | 同 list |
| **board 看板** | 工具栏菜单 | time / title / created / updated | **桶内**按 comparator 排，**桶序（按选项）不动** |
| **跨本收藏（favorites）** | 跟随其生效视图 | 跟随生效视图的通用字段（收藏渲染成上一笔记本的 display_style，无自定义列） | 与对应视图一致；不持久化（无单一 owner 笔记本）。**v1 不单列「收藏时间」字段**，见 §11 |
...
- **分组维度切换（group-by era/year/month）**：`groupTimelineEvents` 已支持 year/month 但页面写死 `era`——这是 group-by 不是 sort，属另一功能，本期不做。
- **多级排序（primary+secondary）**：v1 单键；有需求再扩 `sort` 为数组。
- **收藏时间排序（favorited）**：v1 不做。跨本收藏渲染成上一笔记本的视图（可能是分组的时间线），要按 `favoriteAt` 排必须先决定「收藏视图恒为扁平」还是「分组时按收藏时间沉底」，属独立设计点；届时恢复 `favorited` 字段 + `favoriteAt` 已在索引 DTO 就绪。
- **后端持久化 / 跨设备同步**：如日后要让排序像 `display_style` 那样进 `Topic` 一列，再走幂等 ALTER；当前 localStorage 足够且可逆。
