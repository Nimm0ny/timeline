...
| **W2** | **容器类型 + 去时间化 + 左栏 2 级** | `container_type` 列 + 三类型视图预设（§4）+ 视图切换器按类型过滤 + 废数据能力 gate（换空态）+ 类型可转换；左栏砍到 2 级（书架→容器，去 era 第三级）；笔记去时间化（`year/sort_key/era` 松绑、无日期转正、默认序按类型）。可 sub-split（容器/树 与 去时间化）但一波内收口 | W1 |
| W3 | canvas note_type | 镜像 mindmap（`body_json` + 宽画布顶中栏），自包含版先落 | W2 |
