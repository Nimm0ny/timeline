...
| **W4** ✅ | **链接系统** | `[[ ]]` 解析 + links 表（note-id 寻址）+ backlink 面板 + 悬浮预览（卡加载性能）。**详规 §6**（§6.4 related_event_ids→manual backfill `9ff21e5` + 前端去正向段 `1081fc8`） | W2 |
| **W5** ✅ | **canvas 嵌笔记** | 嵌入节点引用 note-id + 懒渲染 + 视口裁剪。**4 档收在 T0/T1/T2，T3（卡内编辑）砍**——见 §7.9。**详规 §7** | W3 + W4 |
| **W6** ✅ | 改表名 | `timeline_events`→`notes` 全栈 rename：后端符号+schema+guarded 迁移 → 前端 store/util/api-client/组件/目录/props·emits → **wire 契约翻**（BE+FE 原子）。决策：全部重做含 HTTP wire + 连 `timeline` 壳名（无生产、co-deploy 无外部消费者）。13 刀逐一绿、独立 subagent 审 wire 无 asymmetry。**详见 memory `w6-fullstack-rename-plan`**（末刀 `9342e0d`） | 独立、末位 |
...
### 6.4 落地状态（后端 `manual` backfill，已实现）

补齐 §6.4 后端半——`anchor_type` 三态里唯一缺写入器的 `manual` 支。**新 `sync_event_manual_links`**（镜像 `sync_event_embeds`：delete-then-insert 本 source 的 `manual` 行，从 `related_event_ids_json` 反序列化 → 去重 + 剔自关联 → live 目标解析、悬空 `target_event_id NULL`、`context_text="手动关联"`）挂 `create_event`/`update_event`；**一次性 `backfill_manual_links`**（守卫键 `manual_links_backfilled_v1`，镜像 `backfill_event_text_fields`）挂 `main.py` lifespan，把存量 related 边投影进 links 表。**纯 additive**：`relatedEvents`/`relatedEventIds` 前向面包屑**仍读 `related_event_ids_json` 列**——bullet 2（面包屑迁 links）与 bullet 3（前端去手动入口、`关联事件`→backlink）推迟到 **W6 列下线**/独立前端刀；本刀只让手动关系**双向可见**（forward = 列，backward = `manual` backlink）。**偏离 §6.4「一次性」为「持续同步」**：前端手动关联入口未砍，故须每次 save 同步保一致（backfill 只管存量）。pytest `test_links.py` **+6**（manual→backlink / 悬空 NULL / 改列 resync / 自关联去重 / backfill 守卫幂等 / **写入器隔离**——一源兼 wikilink+manual 时 manual 的 delete 不误删 wikilink 行），44 全绿。**注**：当前种子库 0 条 `related_event_ids`，本刀为前向迁移、真实数据 no-op（逻辑由 pytest 兜）。**Review**（独立 subagent，adversarial + 实跑）无阻断项；2 项 P2 已修——① 写入器补 `rid>0` 过滤（对齐 `normalize_related_event_ids`，防 backfill 直读列的 `[0]`/`[-5]` 生 NULL 垃圾行）、② 补写入器隔离测试。

### 6.4 落地状态（前端去正向「关联事件」段，bullet 3 已实现）

`EventDetailPane.vue` 删掉正向「关联事件」段（读态显示 + 编辑态手动关联 picker + kebab「关联事件」项 + 仅正向用的 refs/computed/method/prop/import 与 `.related-results`/`.detail-inline-search`/`.related-result` CSS；`TimelinePage.vue` 去 `:candidate-events` 绑定）。关系统一为 **body `[[`（正向内联）+ `BacklinkPanel`（反向）**，兑现 §6.5「正向链接不进面板（在 body 内联）」。**Frontend-only**：后端 DTO `relatedEventIds`/`relatedEvents` 与 `related_event_ids_json` 列保留至 W6（§9 契约 + `test_timeline_api.py:501` 锁）。**数据安全红线**：保留 `submit()` 发 `relatedEventIds`——删了会让后端 `normalize_related_event_ids` 默认成 `[]`、下次存任何笔记抹掉存量关系（连同 manual backlink），故 draft 正向管道全留、只删 UI。`timelineNotes.js` util（`buildEditorDraft`/`buildReadableDetailGroups`）不动 → 其单测保持绿。验证：test:ui 215、build、agent:check 59；**1920×1080 视觉 QA**（`docs/visual-qa/20260708-related-forward-removal/`）：读/编辑态均无正向段、kebab 无「关联事件」项、`BacklinkPanel` 仍在、读↔编辑零位移、无 console 报错；**端到端**造 A(`relatedEventIds:[1]`)→ 开 A 无正向段、事件 1 的 BacklinkPanel 显 A「手动关联」（闭合 `9ff21e5` 后端环）。**留债**：obsidian spec §7.4/§9 仍写正向段（陈旧，pivot §6.4/§6.5 为新指令，未在本刀改以免动 §9 基准）；bullet 2（`relatedEvents` 面包屑迁 links）随 W6 列下线。
...
| ~~**T3 编辑**~~ ✂ | ~~真 CM6 实例~~ | — | **W5 砍（见 §7.9）**——静息交互＝双击开笔记预览 popover（§7.8 已上线），编辑回笔记本体 |
...
- **W5b-2（已实现 + N=200/40/rich 三 fixture 浏览器 QA，`docs/visual-qa/20260708-w5b2-canvas-t2/`）**：T2 zoom-全文档。纯函数 `computeEmbedTier` 加第 4 档 `full`（屏上像素 ≥ `EMBED_FULL_PX=360` → full；140~360 → preview；240 卡需 zoom≥1.5 才进 T2，是刻意「放大读」）。**并发预算闸落 `CanvasEditor.recomputeTiers`**（单卡纯函数看不到全集）：收集 full 候选按「卡心→视口心」距离排序，仅保留最近 `EMBED_FULL_BUDGET=24` 张为 full、余降 preview（§7.3 软闸；QA 实证 40 候选→24 full + 16 preview，控制台 `T2 over budget: 40 → 24`）。全文数据走钦定链路 `props.resolveDetail`（= `TimelinePage` 下穿的 `timelineStore.ensureEventDetail`，复用 LRU-40 detail cache；`CanvasSurface` 只转发）→ `renderCachedMarkdownToHtml`（复用阅读态缓存渲染器）→ 新 reactive `embedDetailStore`（键 noteId、**又一 off-snapshot 独立通道**，pan/zoom/T2 促发零 DB 写；QA 实证全程零 PUT、`updated_at` 不跳，守 §5.5）。`EmbedCardNode` full 档渲 `.cv-embed-body.markdown-body`（复用全局阅读样式，固定框内滚动，**不放大节点**免脏快照；QA 实证 h1/列表/引用/行内码在卡内成结构且可滚）。**fail-open**：未 resolved / 空正文 / 墓碑一律退回 preview，绝不空白；tombstone 短路不 fetch（免 404 重试）。开图仍 1 次 batch-preview（O(1)），T2 才逐卡 `getEvent`（一卡一取、受 24 顶）。**留债**：detail-cache 陈旧（被嵌笔记别处改动至画布重开才刷，同 §6.8 口径）；显式展开手势未做（仅尺寸驱动，登记 W5b-3+）；预算闸随 fullPx=360 罕触发（真实网格几何最多 ~20 张同屏大卡，闸靠密集 fixture 才验，正合「主策略让 T1 便宜、闸罕触发」）。**Review**（独立 subagent，adversarial）逐写路径确认三硬不变量（off-snapshot 零 PUT、fail-open 绝不空白、固定尺不放大节点）全 hold、无阻断项；4 项 P2 修 3——① 每 id 一次性 attempt（`fullTried` 取代 `fullInFlight`）去 retry-storm、② 卸载后 late-write 加 `graph.value` 守卫、③ 更正 store 有界注释（按板内 full 档去重数有界，非按 24 budget）；P2-4（`is-full` 无专属 CSS）判非缺陷（同既有 `is-shell`，皆 v-if 驱动的状态标记）。
- **W5b-3 → T3 砍（决策见 §7.9）**：卡内 CM6 编辑不做；双击维持＝开笔记预览 popover（§7.8 已上线）。**后端 `related_event_ids`→`manual` backfill 已落**（`9ff21e5`）、**前端去右栏正向段已落**（`1081fc8`）；`relatedEvents` 走 links 表（面包屑迁移）延 **W6**（随列 drop）。

### 7.9 W5 收口 · 砍 T3（决策记录，2026-07-08）

用户拍板：§7.2 的 **T3 卡内 CM6 编辑砍**，W5 收在 **T0/T1/T2 三档 + 双击预览**。§7.2/§7.8 自标「先拍板」的手势冲突就此了结——

- **手势冲突**：§7.2 要 dblclick 激活 T3，但 W5-core 已上线 dblclick＝开笔记预览 popover（§7.8，复用 W4 通道）；二者争同一手势，保留已验证的预览。
- **成本/风险**：CM6 塞进 foreignObject-in-SVG 须与 X6 键盘绑定（Delete/Backspace/Ctrl+Z/Ctrl+A 节点操作）抢事件，且卡内改笔记是一条**画布→笔记本体的新写路径**（并发编辑 / `updated_at` / links resync），边际收益低。
- **已有替代**：被嵌笔记在中栏 feed / 右栏 detail 本就完全可编辑；嵌入卡定位为**只读/预览面**。

**W5 交付（全部已 commit + 浏览器验证，详见 §7.8 与 `docs/visual-qa/`）**：

- W5-core：T0/T1 嵌入卡渲染 + 接线 + 后端 embed links/walker/墓碑（§7.8 已录）。
- W5b-1 `15478b9`（+ 视口 zoom 修 `6747bba` / `11b293d`）：视口裁剪 + T0 壳/T1 预览分档（off-snapshot `canvasTierStore`）。
- W5b-2 `7a6de14`：T2 zoom-全文档（off-snapshot `embedDetailStore` + 24 并发预算闸）。
- §6.4 尾（属链接系统 W4，与 W5 同期收）：后端 manual backfill `9ff21e5` + 前端去正向段 `1081fc8`。

**§7.7 验收**：性能 QA（N=200 开图 / pan-zoom 帧率 / 进出视口无抖 / T2 预算不超）由 W5b-1（`docs/visual-qa/20260708-w5b-canvas-culling/`）+ W5b-2（`docs/visual-qa/20260708-w5b2-canvas-t2/`）归档兑现；三档全程守 §5.5 off-snapshot——pan/zoom/分档零 `updated_at` 污染。

**留债**：`obsidian-minimal-implementation-spec.md` §7.4/§9 仍写正向「关联事件」段 + T3 卡内编辑，陈旧（pivot §6.4/§6.5 + 本节为新指令，登记债，非本刀改）；detail-cache 陈旧（被嵌笔记别处改动至画布重开才刷，§6.8 口径）；显式展开手势未做（T2 仅尺寸驱动）。

> 注：§7.2 T3 行、§7.3 的 T2→T3／T3=1 规则、§7.8「T2/T3 归 W5b」均为**原设计**表述；T3 部分以本节「砍」为准。
